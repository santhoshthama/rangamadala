...
CREATE TABLE IF NOT EXISTS `payments` (
  ...
  `payment_type` enum('advance','final','full') NOT NULL COMMENT 'Type of payment: advance (before work), final (after work), full (complete)',
  ...
  `payment_stage` enum('before_confirmation','after_completion') DEFAULT 'after_completion' COMMENT 'When payment should happen',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
